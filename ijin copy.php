...
$stmt = $pdo->prepare('
    SELECT 
        jatah_cuti_tahunan,
        COALESCE((
            SELECT SUM(DATEDIFF(tanggal_selesai, tanggal_mulai) + 1) 
            FROM pengajuan_cuti 
            WHERE id_pegawai = ? 
    ...
    WHERE id_pegawai = ?
');
$stmt->execute([$user['id_pegawai'], $user['id_pegawai']]);
$data_cuti = $stmt->fetch(PDO::FETCH_ASSOC);

$jatah_cuti = $data_cuti['jatah_cuti_tahunan'];
$cuti_dipakai = $data_cuti['cuti_dipakai'];
$sisa_cuti = $jatah_cuti - $cuti_dipakai;

// Hitung total cuti sakit
$stmt = $pdo->prepare('
    SELECT COALESCE(SUM(DATEDIFF(tanggal_selesai, tanggal_mulai) + 1), 0) as total_sakit
    FROM pengajuan_cuti 
    WHERE id_pegawai = ? 
    AND jenis_cuti = "sakit" 
    AND status = "disetujui"
');
$stmt->execute([$user['id_pegawai']]);
$data_sakit = $stmt->fetch(PDO::FETCH_ASSOC);
$total_sakit = $data_sakit['total_sakit'];
            ...
            if (!$error) {
                try {
                    ...
                    $stmt = $pdo->prepare('
                        ...
                        (id_pegawai, jenis_cuti, tanggal_mulai, tanggal_selesai, link_data_dukung, status) 
                        VALUES (?, ?, ?, ?, ?, "pending")
                    ');
                    $stmt->execute([
                        $user['id_pegawai'],
                    ...
                    $stmt->execute([$user['id_pegawai'], $user['id_pegawai']]);
                    $data_cuti = $stmt->fetch(PDO::FETCH_ASSOC);
                    $sisa_cuti = $data_cuti['jatah_cuti_tahunan'] - $data_cuti['cuti_dipakai'];
            ...
            <div class="bg-white rounded-2xl shadow-lg p-6">
                ...
                <div class="mt-6 p-4 bg-purple-50 rounded-lg">
                    ...
                    <ul class="text-purple-700 text-sm space-y-1">
                        <li>• Cuti tahunan akan mengurangi sisa cuti tahunan</li>
                ...
                <form method="POST" class="space-y-6">
                    ...
                    <div class="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
                        ...
                        <p class="text-yellow-700 text-sm mt-2" id="info-hari-cuti">
                            Pilih tanggal mulai dan selesai untuk melihat jumlah hari cuti
                        </p>
                    </div>
    ...
    <script>
        ...
        function hitungHariCuti() {
            ...
            if (tanggalMulai && tanggalSelesai) {
                ...
                if (selisihHari > 0) {
                    infoElement.innerHTML = `<strong>${selisihHari} hari</strong> cuti (${tanggalMulai} sampai ${tanggalSelesai})`;
                    infoElement.className = 'text-green-700 text-sm mt-2';
                } else {
                    infoElement.innerHTML = 'Tanggal selesai harus lebih besar atau sama dengan tanggal mulai';
        ...
        document.querySelector('form').addEventListener('submit', function(e) {
            ...
            if (jenisCuti === 'tahunan') {
                const mulai = new Date(tanggalMulai);
                const selesai = new Date(tanggalSelesai);
                const selisihHari = Math.ceil((selesai.getTime() - mulai.getTime()) / (1000 * 3600 * 24)) + 1;
                ...
                if (selisihHari > sisaCuti) {
                    ...
                    Swal.fire({
                        ...
                        text: `Anda mengajukan ${selisihHari} hari cuti, tetapi sisa cuti tahunan hanya ${sisaCuti} hari`,
                        confirmButtonText: 'OK'
                    });
                    return false;
                }
